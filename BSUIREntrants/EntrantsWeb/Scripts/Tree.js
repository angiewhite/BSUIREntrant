	...
	function load(node) {
		...
		function onLoaded(data) {
			...
			for(var i = 0; i < data.length; i++) {
                ...
                li.innerHTML = '<div class="Expand"></div><input type="checkbox"/><div class="Content">' + child.name + "(" + child.count + ")" + '</div>'
				if (child.count != 0) {
					li.innerHTML += '<ul class="Container"></ul>'
				}
				node.getElementsByTagName('UL')[0].appendChild(li)
            }
            if (data.length) node.getElementsByTagName("UL")[0].id = data[0].type
		...
		$.ajax({
            url: url,
            data: { id: node.id, typeName: node.parentNode.id },
			dataType: "json",
			success: onSuccess,
			error: onAjaxError,
			cache: false
